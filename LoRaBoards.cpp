...
bool displayGNSSInfo() {
  // Working Startup Screen
  Wire.beginTransmission(DISPLAY_ADDR);
  if (Wire.endTransmission() == 0) {
    u8g2 = new DISPLAY_MODEL(U8G2_R0, U8X8_PIN_NONE);
    u8g2->begin();
    u8g2->clearBuffer();
    u8g2->setFont(u8g2_font_inb19_mr);
    u8g2->drawStr(0, 30, "Austin's");
    u8g2->setFont(u8g2_font_inb19_mr);
    u8g2->drawStr(30, 60, "LoRa!");
    u8g2->sendBuffer();
    u8g2->setFont(u8g2_font_fur11_tf);
    delay(3000);
    return true;
  ...
  uint32_t gnssBaudRate = 38400;

  //displayString Test
  displayString("Fuck");  // Not working
  delay(3000);
  ...
  while (gNSS.begin(SerialGPS) == false)  //Connect to the u-blox module using Wire port
  {
    Wire.beginTransmission(DISPLAY_ADDR);
    if (Wire.endTransmission() == 0) {
      u8g2->begin();
